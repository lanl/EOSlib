...
#include "HotSpotHE.h"
//
double ArrheniusRT::DLAMBDA = 0.01;     // default for dlambda
double ArrheniusRT::DLAMBDA_DT = 0.1;   // default for dlambda_dt
double ArrheniusRT::RMIN = 1.0e-10;     // minimum rate for default T_cutoff
double ArrheniusRT::TOL  = 1.0e-6;      // default for tol
//
//
ArrheniusRT::ArrheniusRT() : HErate("ArrheniusRT"),
                n(1.0), k(0.0),T_a(0.0),T_cutoff(0.0),T_max(11604.5),
                m(0.0), T_m(1.0),
                dlambda(ArrheniusRT::DLAMBDA), dlambda_dt(ArrheniusRT::DLAMBDA_DT),
                tol(ArrheniusRT::TOL), dt_min(0.0), source_type(3), integrate(1)
{
    // Null
}

ArrheniusRT::~ArrheniusRT()
{
    // Null
}

HErate *ArrheniusRT::Copy(PTequilibrium *eos)
{
    ArrheniusRT *rate   = new ArrheniusRT;
    rate->n           = n;
    rate->k           = k;
    rate->T_a         = T_a;
    rate->m           = m;
...
IDOF *ArrheniusRT::Idof()
{
    return new HotSpotHE;
}

void ArrheniusRT::PreInit(Calc &calc)
{
    n = 1.; // first order
    k = T_a     = 0.0;
    m           = 0.0;
...
int ArrheniusRT::PostInit(Calc &, DataBase *)
{
    //if( n<0.0 || n>1. )
    if( n<0.0 )
        return 1;
...
void ArrheniusRT::PrintParams(ostream &out)
{
    out << "\t      n      = " << n           << "\n"
        << "\t      k      = " << k           << "\n"
        << "\t      T_a    = " << T_a         << "\n"
...
int ArrheniusRT::ConvertParams(Convert &convert)
{
