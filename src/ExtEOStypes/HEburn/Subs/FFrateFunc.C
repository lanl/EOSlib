...
double FFrate::Xpop(double p)
{
   double lnx = (a*log(10.) - log(p/Pref))/b;
   return xref*exp(lnx);
}

double FFrate::Rate(double p)
{
   if( p < hug.ws0.P )
       return 0.0;
   p = min(p,hug.CJ.P);
...
double FFrate::Rate(double lambda, double p)
{
...
int FFrate::Rate(double V, double e, const double *z, double *zdot)
{
    if( z[0] < 1.0 )
    {
        double p = P(V,e,z[0]);
...
int FFrate::TimeStep(double V, double e, const double *z, double &dt)
{
    dt = 0.0;
    if( z[0]<0. || 1.<=z[0] )
...
int FFrate::Integrate(double V, double e, double *z, double dt)
{
    if( HE == NULL )
        return 1;
    if( dt<=0.0 || z[0]>=1. )
...
double FFrate::Dt(double V, double e, const double *z, double lambda)
{
    if( HE == NULL )
        return EOS::NaN;
