...
double HotSpotRate2::s0(double Ps, double Ts)
{
    double Ths = Ts + a*Ps;
    double x = erfc( (Ti-Ths)/sigma );
    return b*pow(x,(1./3.));
}

int HotSpotRate2::Rate(double V, double e, const double *z, double *zdot)
{
    double     Ps = z[0];
    double     Ts = z[1];
    double     ts = z[2];
...
int HotSpotRate2::TimeStep(double V, double e, const double *z, double &dt)
{
    dt = 0.0;
    if( source_type == 2 )
...
int HotSpotRate2::Integrate(double V, double e, double *z, double dt)
{
    if( HE == NULL )
        return 1;
    if( dt <= 0.0 )
...
double HotSpotRate2::Dt(double V, double e, const double *z, double lambda)
{
