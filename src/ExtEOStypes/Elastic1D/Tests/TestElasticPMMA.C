...
int main(int, char **argv)
{
    ...
    Elastic1D *EP = dynamic_cast<Elastic1D *>(eos);
    if( EP == NULL )
    ...
    **************/
	...
	WaveStateLabel(cout) << " " << Center(u_form, "c")
                         ...
                         << " " << Center(P_form, "Phydro")
                         << " " << Center(P_form, "Pshear")
                         << " " << Center(P_form, "Pdev")
                         << "\n";
    ...
    for(i=0; i<=nsteps; i++ )
	{
        ...
        *******************/
        double eps = EP->Eps_el(w1.V);
        double Pdev = EP->Pdev(w1.V,w1.e,eps);
        double Pshr = EP->Pshear(w1.V,w1.e,eps);
        double Phyd = EP->Phydro(w1.V,w1.e,eps);
        double c    = EP->c(w1.V,w1.e);
		cout << w1
             << " " <<   u_form << c 
             << " " <<   T_form << EP->EOS::T(w1) 
             << " " <<   S_form << EP->EOS::S(w1) 
