...
int main(int, char **argv)
{
    ...
    double e = NaN;
    double dt = NaN;
    double eps_pl = 0;   // eps < 0 in compression
    double nf = 20;
    double tau0 = NaN;
    ...
    if( !std::isnan(V) && !std::isnan(e) )
    {
        cout << "EOS\n"
             ...
             << "          S = " << eos->S(V,e)         << "\n"
             << "          c = " << eos->c(V,e)         << "\n";
        VMrate *EP = dynamic_cast<VMrate *>(eos);
        if( EP == NULL )
        ...
        if( EP->Rate(V,e, z, zdot) != 2 )
            cerr << Error("Rate failed") << Exit;            
        cout << "(d/dt)eps_pl = " << zdot[0]
             << ", (d/dt)tauinv = " << zdot[1] << "\n";
        double dt_step;
