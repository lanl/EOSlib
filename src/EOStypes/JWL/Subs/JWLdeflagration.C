...
Deflagration *JWL::deflagration(const HydroState &state, double P0)
{
    Deflagration *S = new JWLdeflagration(*this);
    if( S->Initialize(state,P0) )
    {
        delete S;
        S = NULL;
        EOSerror->Log("JWL::deflagration", __FILE__, __LINE__, this,
               "Initialize failed");
    }
    return S;
}
...
JWLdeflagration::JWLdeflagration(JWL &gas, double pvac) : Deflagration(gas,pvac)
{
...
JWLdeflagration::~JWLdeflagration()
{
    // Null
}

int JWLdeflagration::InitCJ()
{
    double Vmin = V0;
    P1 = eos->P(Vmin,e0);
    double fmin = P1 - P0;
    ...
    {
        P0 = EOS::NaN;
        eos->ErrorHandler()->Log("JWLdeflagration::InitCJ", __FILE__, __LINE__,
                eos, "fmin <= 0\n" );
        return 1;
    }
    double Vmax;
...
double JWLdeflagration::f(double var)
{
    V1 = var;
    double dV = V0 - V1;
    switch (fzero)
...
int JWLdeflagration::P(double p1,  int dir, WaveState &wave)
{
    ...
    if( Status() )
    {
        eos->ErrorHandler()->Log("JWLdeflagration::P", __FILE__, __LINE__,
...
int JWLdeflagration::u(double u1,  int dir, WaveState &wave)
{
    ...
    if( Status() )
    {
        eos->ErrorHandler()->Log("JWLdeflagration::u", __FILE__, __LINE__,
             eos, "inverse failed with status: %s\n", ErrorStatus() );
        return 1;
    }  
    (void) f(V);                // evaluate state
...
int JWLdeflagration::u_s(double us, int dir, WaveState &wave)
{
    ...
    if( Status() )
    {
        eos->ErrorHandler()->Log("JWLdeflagration::u_s", __FILE__, __LINE__,
             eos, "inverse failed with status: %s\n", ErrorStatus() );
        return 1;
    }  
    (void) f(V);                // evaluate state
